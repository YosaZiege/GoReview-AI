...
func BuildDeepPrompt(summary core.ASTSummary, existing []core.Finding) string {
	...
	sb.WriteString("CRITICAL: Your ENTIRE response must be a single raw JSON array. No markdown, no prose, no explanation before or after.\n")
	sb.WriteString("Start your response with [ and end with ]. Return [] if no smells found.\n\n")
	sb.WriteString("Each element:\n")
